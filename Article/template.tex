...
\Figure{0.85\textwidth}{images/PLL.png}{Схема классической системы ФАПЧ}
...
&a = \tau_{p1}+\tau_{p2} \text{,} \quad b = \tau_{p1}\tau_{p2} \text{,} \quad 0 < \tau_{p1} < 1 \text{,} \quad 0 < \tau_{p2} <1
 \end{aligned}
\end{equation}
Поскольку $\text{det}(sI-A) = s^2 + ab^{-1}s + b^{-1}$, матрица $A$~--- устойчива. Очевидно, что пара $\left(A, b\right)$ вполне управляема. Подставим \eqref{filter1} в \eqref{first_th_eq} и перенесем все в левую часть неравенства. Сделаем замену $t = s^2$. Для максимизации $\nu$ положим $\varkappa = 1$ и $\varepsilon = 1 - \tau - \delta$, тогда \eqref{first_th_eq} принимает вид
\begin{equation}\label{ineq1} 
 \begin{aligned}
&\widehat At^2 + \widehat Bt + \widehat C \geqslant 0 \text{,}\quad \forall t \in \mathbb{R_+}\\
&\widehat A = \tau b^2 \text{,}\quad
\widehat B = \tau a^2-2 \tau b - \delta b^2 \text{,}\quad
\widehat C = - b+\tau-\delta a^2 + 2\delta b
 \end{aligned}
\end{equation}
Потребуем $\widehat A \geqslant 0$ и $\widehat C \geqslant 0$, тогда $\widehat B \geqslant 0$. 
\Figure{\textwidth}{images/agregated.jpg}{Численное и аналитическое решения для фильтра \eqref{filter1}}

Максимизируем $\nu$, при условии $\widehat C \geqslant 0$. Рассмотри границу допустимой области, т.\,е. $\widehat C = 0$. Из второго условия теоремы и $\delta = \frac{1-b}{2(a^2 - 2b + 1)}$ получаем
...
\beta_1 < \beta_2 < 1
 \end{aligned}
\end{equation}
Поскольку $\text{det}(sI-A) = s^2 + \alpha_1\alpha_2^{-1}s + \alpha_2^{-1}$, матрица $A$~--- устойчива. Очевидно, что пара $\left(A, b\right)$ вполне управляема. Подставим \eqref{filter3} в \eqref{first_th_eq} и перенесем все в левую часть неравенства. Для максимизации $\nu$ положим $\tau = 0$, $\varkappa = 1$ и $\varepsilon = 1-\delta$. Тогда \eqref{first_th_eq} принимает вид
 \begin{equation}\label{filter3-3}
 \begin{aligned}
 &\widehat At + \widehat B \geqslant 0 \text{,}\quad \forall t \in \mathbb{R_+}\\
 &\widehat A = \alpha_2^2\beta_2 - \alpha_2^2\delta - \alpha_2^2\beta_2^2 + \alpha_2^2\beta_2^2\delta\\
 &\widehat B = \alpha_2\beta_2 - \alpha_2 + 2\alpha_2\delta + \alpha_1^2\beta_1 - \alpha_1^2\delta - \alpha_1^2\beta_1^2 + \alpha_1^2\beta_1^2\delta - 2\alpha_2\beta_2\delta
 \end{aligned}
\end{equation}
Выполнение \eqref{restriction-1} влечет $\widehat A > 0$. Максимизируем $\nu$ на границе области \eqref{filter3-3}, т.\,е. $\widehat B = 0$.
...
\delta = \frac{\alpha_1^2(1-\beta_1)\beta_1 - \alpha_2(1-\beta_2)}{\alpha_1^2(1-\beta_1^2) - 2\alpha_2(1-\beta_2)}
 \end{aligned}
 \end{equation}
Потребуем $\alpha_1^2 > \frac{\alpha_2(1-\beta_2)}{\beta_1(1-\beta_1)}$. Это условие гарантирует положительность  \eqref{filter3_positive_numerator}. Из второго условия теоремы и \eqref{filter3_positive_numerator}
 \begin{equation}
 \begin{aligned}
