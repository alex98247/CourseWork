...
Хорошо известна система дифференциальных уравнений, описывающих ФАПЧ \cite{kuznetsov_article}, рис.~\ref{pll_circuit}
 ...
 \end{aligned}
\end{equation}
где $A$~--- постоянная матрица $n \times n$, $B$ и $C$ постоянные $n$~--- мерные векторы, $D$~--- константа, $x(t)$~--- $n$-мерный вектор состояний системы. $W(s)=C^T \left(A-sI\right)^{-1}B - D$~--- передаточная функция фильтра. $K_{vco}$~--- коэффициент передачи, $\gamma$ определяется следующим образом
