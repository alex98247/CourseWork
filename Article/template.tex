...
\beta_1 < \beta_2 < 1
 \end{aligned}
\end{equation}
Поскольку $\text{det}(sI-A) = s^2 + \alpha_1\alpha_2^{-1}s + \alpha_2^{-1}$, матрица $A$~--- устойчива. Очевидно, что пара $\left(A, b\right)$ вполне управляема. Для максимизации $\nu$ подставим \eqref{filter3} в \eqref{first_th_eq} и перенесем все в левую часть неравенства. Положим $\tau = 0$, и не умаляя общности можем считать $\varkappa = 1$. В результате преобразований, первое условие теоремы \eqref{first_th_eq} принимает следующий вид
