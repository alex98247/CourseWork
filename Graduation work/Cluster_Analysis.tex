...
&+ (1-\varepsilon - \delta)  \geqslant 0, \quad \forall t \in \mathbb{R_+}
 \end{aligned}
\end{equation}
Для того, что бы выполнялось \eqref{second_condition_tau_zero} потребуем положительность коэффициентов: 
...
Из \eqref{filter3-restriction1} выразим $\delta$:
 \begin{align}
&\delta \leqslant \frac{\beta_2}{1+\beta_2} \\
&\delta \leqslant \frac{\alpha_1^2(1-\beta_1)\beta_1 - \alpha_2(1-\beta_2)}{\alpha_1^2(1-\beta_1^2) - 2\alpha_2(1-\beta_2)} \label{free_coef}
 \end{align}
Условие \eqref{restriction-1} гарантирует, что функция \eqref{free_coef} как функция от  $\beta_1$ строго возрастает, на допустимой области параметров. Положим в \eqref{free_coef} $\beta_1 = \beta_2$, тогда:
 \begin{align}
\frac{\alpha_1^2(1-\beta_1)\beta_1 - \alpha_2(1-\beta_2)}{\alpha_1^2(1-\beta_1^2) - 2\alpha_2(1-\beta_2)} < \frac{\alpha_1^2\beta_2 - \alpha_2}{\alpha_1^2 - 2\alpha_2} < \frac{\beta_2}{1+\beta_2}
 \end{align}
...
\alpha_1^2 > \frac{\alpha_2(1-\beta_2)}{\beta_1(1-\beta_1)}
 \end{aligned}
 \end{equation}
Покажем, что это условие гарантирует положительность знаменателя \eqref{filter3_positive_numerator}:
\begin{equation}
 \begin{aligned}
