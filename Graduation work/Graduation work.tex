...
\draw[->] (a.east) -- (loop_filter.west) node[midway,above,inner sep=1pt] {sin($\omega_{e}^{free}t$)};
...
Пусть все нули функции $\upsilon_e(\theta_e) - \gamma$ изолированы, пара $(A, B)$ вполне управляема, все собственные значения матрицы $A$ имеют отрицательные вещественные части и существуют числа $\varepsilon > 0, \delta > 0, \tau \geqslant 0$, и $\varkappa$, такие что имеют место неравенства:
 \begin{flalign}
 &\operatorname{Re}\left( \varkappa W(ix)- \varepsilon\left[W(ix)\right]^2- \right.\notag &&\\
&\left. \hspace{100pt minus 1fil}-\tau\left[ 	
\overline{W(ix)}-\mu_1^{-1} ix \right]\left[W(ix)+\mu_2^{-1} ix \right]\right) \geqslant \delta, \forall x \in \mathbb{R} \label{first_th_eq0}\\
...
\bibitem{mouna} Mouna~B.\:H., Lassaad~S. Digital  Phase  Locked Loop  Induction  Motor  Speed  Controller:  Design  and  Experiments // Leonardo Electronic Journal of Practices and Technologies. 2012. Vol.~20, P.~159--174.  
...
\bibitem{kaplan} Kaplan~D.\:E., Hegarty~J.\:C. Understanding GPS: Principles And Applications / Boston: Artech House, 2006. P.~166--168.
